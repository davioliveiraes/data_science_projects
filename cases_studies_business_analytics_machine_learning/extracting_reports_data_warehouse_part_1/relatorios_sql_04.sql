...
-- Stored Procedure para gravar em uma tabela os clientes que geram faturamento acima de 10000
...
BEGIN
	...
	-- Insere na tabela de relatório somente os clientes
...
AS $$
BEGIN
	-- Verifica se o NomeCliente está no formato correto ('Cliente Konoha' seguido de números)
	IF NomeCliente ~ 'Cliente Konoha' THEN
		RETURN QUERY
		SELECT
		...
		ORDER BY
			p.NomeCategoria;
	ELSE
		RAISE EXCEPTION 'O nome do cliente não está no formato esperado: ''Cliente Konoha [0-9]+''';
	END IF;
END;
$$;

-- Executa a função
SELECT * FROM dw.fn_relatorio_vendas_categoria('Cliente Konoha 10');
SELECT * FROM dw.fn_relatorio_vendas_categoria('Cliente Konoha 42');
SELECT * FROM dw.fn_relatorio_vendas_categoria('Cliente Konoha');
