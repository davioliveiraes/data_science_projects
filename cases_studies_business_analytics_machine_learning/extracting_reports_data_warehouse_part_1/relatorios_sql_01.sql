-- Estudo de Caso 2 - Extraindo Relatórios do Data Warehouse
...
SELECT Nome, Preco, NomeCategoria FROM dw.Produto WHERE NomeCategoria = 'Categoria 2';

-- Funções de agregação
SELECT COUNT(Preco), MIN(Preco), SUM(Preco), AVG(Preco), MAX(Preco) FROM dw.Produto;

-- Funções de agregação e arrendodamento
SELECT COUNT(Preco), MIN(Preco), SUM(Preco), AVG(Preco), MAX(Preco) FROM dw.Produto;

-- Funçções de agregação e arredondamento, com alias
...
SELECT COUNT(Preco) AS contagem,
			...
			ROUND(AVG(Preco), 2) AS media,
			MAX(Preco) AS valor_maximo
FROM dw.Produto WHERE NomeCategoria = 'Categoria 3';

-- (SQL - Nível Intermediário)

-- Listar os produtos da Categoria 3 e preço maior que 400
SELECT Nome, Preco, NomeCategoria FROM dw.Produto 
WHERE NomeCategoria = 'Categoria 3' AND Preco > 400;

-- Listar os produtos da Categoria 3 ou preço maior que 400
SELECT Nome, Preco, NomeCategoria FROM dw.Produto 
WHERE NomeCategoria = 'Categoria 3' OR Preco > 400;

-- Listar os produtos da Categoria 3 e preço maior que 400, ou apenas preço maior que 400
SELECT Nome, Preco, NomeCategoria FROM dw.Produto 
WHERE (NomeCategoria = 'Categoria 3' AND Preco > 400) OR Preco > 400;

-- Listar nome, categoria e quantidade vendida para cada produto
SELECT p.nome, p.nomecategoria, v.quantidadevendida 
FROM dw.produto p JOIN dw.Vendas v ON v.produtoid = p.produtoid;
...
FROM dw.Produto p JOIN dw.Vendas v ON v.produtoid = p.produtoid JOIN dw.data d ON v.dataid = d.dataid;
...
-- Listar nome, categoria, quantidade vendida, data de venda, país do cliente e nome do distribuidor para cada produto
...
-- Listar nome, categoria, quantidade vendida, data de venda, país do cliente e nome do distribuidor, para cada produto que atenda esta regra: produtos da Categoria 3 e preço maior que 400, ou apenas preço maior que 400
SELECT p.nome, p.nomecategoria, v.quantidadevendida, dt.data, c.paiscliente, d.nomedistribuidor 
FROM dw.Produto p JOIN dw.Vendas v ON v.produtoid = p.produtoid 
JOIN dw.data dt ON v.dataid = dt.dataid 
JOIN dw.cliente c ON v.clienteid = c.clienteid 
...
-- Soma de Faturamento por Cidade do Cliente
SELECT c.CidadeCliente, SUM(v.Faturamento) AS TotalFaturamento
FROM dw.Vendas v
JOIN dw.Cliente c ON v.ClienteID = c.ClienteID
GROUP BY c.CidadeCliente;

-- Faturamento Média por Dia da Semana, Ordenado Por Faturamento e Somente Para os Dias Úteis
...
-- Total de Vendas e Custos de Frete por País do Cliente e Categoria de Produto, para o Ano de 2024
...
SELECT p.nomecategoria,
	   ...
	   ROUND(AVG(v.quantidadevendida), 2) AS MediaQuantidadeVendidade,
...
JOIN dw.cliente c ON v.clienteid = c.clienteid
JOIN dw.distribuidor d ON v.distribuidorid = d.distribuidorid
WHERE ((c.CidadeCliente = 'New York' AND Preco > 400) OR Preco > 400)
...
-- Total de Vendas e Custo de Frete por País do Cliente e Categoria de Produto, para o Ano de 2024 somente se o custo total de frete for maior do que 30.000
SELECT c.PaisCliente, p.NomeCategoria, SUM(v.QuantidadeVendida) AS TotalVendas, SUM(v.CustoFrete) AS TotalCustoFrete
FROM dw.Vendas v
JOIN dw.Cliente c ON v.ClienteID = c.ClienteID
JOIN dw.Produto p ON v.ProdutoID = p.ProdutoID
...
-- Para ilustrar o uso de subquery no contexto deste modelo de dados,
-- Vamos criar uma consulta que identifique os clientes que realizaram compras acima da média.
-- Está consulta irá calcular o total de faturamento por cliente e, em seguida, utilizar uma subquery para selecionar apenas
-- Aqueles clientes cujo o total de faturamento esteja acima da média.
...
JOIN dw.Cliente c ON v.ClienteID = c.ClienteID
GROUP BY c.Nome
-- Retorna somente os cliente cuja total de faturamento tenha sido acima da média
HAVING SUM(v.Faturamento) > (
